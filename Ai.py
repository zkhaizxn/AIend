...
def load_dishes_from_mysql(floor=None):
    ...
    try:
        if floor and floor != "全部":
            query = "SELECT name, calories, protein, fat, taste, health_goal, allergens, price, category FROM dishes WHERE floor = %s"
...
def load_user_profile(user_id):
    ...
    try:
        with conn.cursor(pymysql.cursors.DictCursor) as cursor:
            ...
            row = cursor.fetchone()
            if row:
                if row.get('extra_info') and isinstance(row['extra_info'], str):
        ...
        with conn.cursor() as cursor:
            ...
            if exists:
                ...
                if set_clauses:
                    ...
                    sql = f"UPDATE user_profiles SET {', '.join(set_clauses)}, updated_at = NOW() WHERE user_id = %s"
...
def save_conversation_to_db(user_id, messages, timestamp=None):
    ...
    try:
        with conn.cursor() as cursor:
            cursor.execute(
                "SELECT id FROM user_conversation_history WHERE user_id = %s AND messages = %s LIMIT 1",
                (user_id, messages_json)
            )
            if cursor.fetchone():
                return
            cursor.execute(
                "INSERT INTO user_conversation_history (user_id, timestamp, messages, preview) VALUES (%s, %s, %s, %s)",
...
def load_user_conversations(user_id):
    ...
    try:
        with conn.cursor(pymysql.cursors.DictCursor) as cursor:
            cursor.execute(
                "SELECT id, timestamp, preview, messages FROM user_conversation_history WHERE user_id = %s ORDER BY created_at DESC",
                (user_id,)
            )
            rows = cursor.fetchall()
            for row in rows:
...
def delete_all_conversations_of_user(user_id):
    ...
    try:
        with conn.cursor() as cursor:
            cursor.execute("DELETE FROM user_conversation_history WHERE user_id = %s", (user_id,))
            conn.commit()
        ...
        with placeholder.container():
            with st.spinner("🤔 AI正在分析..."):
                try:
                    ...
                    # 如果没有匹配到推荐标记，且用户消息中包含食物相关关键词，则自动构造一个推荐请求
